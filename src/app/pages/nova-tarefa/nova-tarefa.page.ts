...
export class NovaTarefaPage {
  ...
  constructor(private googleTasks: GoogleTasksService, private router: Router, private native: NativeService) {}
  ...
  async salvar() {
    ...
    try {
      ...
      await this.native.pedirPermissaoNotificacao();
      await this.native.agendarNotificacao(
        'Tarefa criada!',
        `"${this.titulo}" foi adicionada com sucesso.`
      );
